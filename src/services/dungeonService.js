...
const getCharacter = async (characterId = '') => {
  const result = await fetch(`http://${serverDomain}/api/data/pc/${characterId}?detail=true`);
  return result.json();
};
...
const saveCharacter = async (character) => {
  try {
    await fetch(`http://${serverDomain}/api/data/pc?uid=${character.info.id}`,
      {
        method: 'POST',
...
const getCharactersByOwner = async () => {
  const result = await fetch(`http://${serverDomain}/api/data/pc?detail=true`);
  return result.json();
};
...
const getRace = async (id) => {
  const result = await fetch(`http://${serverDomain}/api/races/${id}`);
  return result.json();
};
